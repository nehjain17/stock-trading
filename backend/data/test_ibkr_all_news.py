...
- Python 3.9+
- Per-symbol dedup
- Only outputs last N days (default 3 in main())
- Suppresses the expected Error 162 "scanner subscription cancelled"
- Cleans IBKR headline metadata prefixes like: {A:800015,...}*
"""
...
# --- Headline cleaning (removes IBKR metadata prefix) ---
...
def clean_headline(text: str) -> str:
    if not text:
        return ""
    s = str(text).strip()
    # remove "{A:...}*" prefix (sometimes repeated)
    for _ in range(3):
    ...
    def historicalNews(self, reqId, timeStr, providerCode, articleId, headline):
        ...
        self.news_rows.append(
            NewsRow(
                ...
                headline=clean_headline(headline),  # <-- CLEANED HERE
            )
        )
