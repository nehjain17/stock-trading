...
# Put your CSVs inside backend/data/
DATA_DIR = BASE_DIR / "data"
DATA_DIR.mkdir(exist_ok=True)

SCREENER_FILES: Dict[str, Tuple[str, str]] = {
...
def to_dt_utc(df: pd.DataFrame) -> pd.Series:
    # Handles both IBKR formats you showed: time_utc or time
...
def pick_existing(filename: str) -> Optional[Path]:
    p = DATA_DIR / filename
...
def get_paths(screener: str) -> Tuple[Path, Path]:
    ...
    scanner_path = pick_existing(scanner_name) or (DATA_DIR / FALLBACK_SCANNER)
    news_path = pick_existing(news_name) or (DATA_DIR / FALLBACK_NEWS)
...
def api_screener(screener: str = "most_active", days: int = 3, search: str = ""):
    ...
    if not scanner_path.exists():
        return {"error": f"Missing scanner CSV: {scanner_path.name} in {DATA_DIR}"}
    if not news_path.exists():
        return {"error": f"Missing news CSV: {news_path.name} in {DATA_DIR}"}
    ...
    if search:
        news_recent = news_recent[
            news_recent["headline"].astype(str).str.contains(search, case=False, na=False)
        ]
...
def api_headlines(screener: str = "most_active", days: int = 3, symbol: str = "", search: str = ""):
    ...
    if not news_path.exists():
        return {"symbol": symbol, "days": int(days), "items": []}
    ...
    df = news_df[
        (news_df["published_at_utc"] >= cutoff)
        & (news_df["symbol"] == symbol.upper().strip())
    ].copy()
    ...
    return {"symbol": symbol.upper().strip(), "days": int(days), "items": items}
