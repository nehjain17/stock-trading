#!/usr/bin/env python3
"""
Fetch shortable shares data from IBKR for all US stocks.
Reads symbols from us_stock_symbols.csv and saves results to shortable_data.csv
"""
import pandas as pd
import asyncio
import os
from dotenv import load_dotenv
from ib_insync import IB, Stock
import time
import sys

# Load environment variables
load_dotenv()

# IBKR Connection settings
IBKR_HOST = os.getenv('IBKR_HOST', '127.0.0.1')
IBKR_PORT = int(os.getenv('IBKR_PORT', 7497))
IBKR_CLIENT_ID = int(os.getenv('IBKR_CLIENT_ID', 1))

async def connect_ibkr():
    """Connect to Interactive Brokers in read-only mode"""
    try:
        ib = IB()
        ib.connect(IBKR_HOST, IBKR_PORT, clientId=IBKR_CLIENT_ID, readonly=True)
        await asyncio.sleep(0.5)
        print("✓ Connected to IBKR")
        return ib
    except Exception as e:
        print(f"✗ IBKR Connection Error: {e}")
        print("Make sure TWS/IB Gateway is running with API enabled")
        return None

async def get_shortable_shares(ib, ticker):
    """Fetch shortable shares for a stock"""
    try:
        import math
        contract = Stock(ticker, 'SMART', 'USD')
        
        # Request market data which includes shortable shares
        ib.reqMarketDataType(4)  # Delayed data
        ib.reqMktData(contract, '', False, False)
        await asyncio.sleep(1.0)  # Wait for data
        
        # Get the ticker object which contains shortable info
        ticker_obj = ib.ticker(contract)
        shortable = ticker_obj.shortableShares if hasattr(ticker_obj, 'shortableShares') else None
        
        # Handle NaN values
        if shortable is None or (isinstance(shortable, float) and math.isnan(shortable)):
            shortable = 0
        else:
            try:
                shortable = int(shortable)
            except (ValueError, TypeError):
                shortable = 0
        
        # Cancel market data subscription
        try:
            ib.cancelMktData(contract)
        except:
            pass
        
        return {
            'symbol': ticker,
            'shortable_shares': shortable
        }
    except Exception as e:
        return None

async def scan_all_stocks(symbols):
    """Scan all stocks for shortable shares"""
    ib = await connect_ibkr()
    if not ib:
        return []
    
    results = []
    total = len(symbols)
    
    print(f"\nScanning {total} stocks for shortable shares...")
    print("This will take some time (~1-2 seconds per stock)\n")
    
    for idx, symbol in enumerate(symbols, 1):
        try:
            data = await get_shortable_shares(ib, symbol)
            if data:
                results.append(data)
                
                # Progress update every 10 stocks
                if idx % 10 == 0:
                    print(f"Progress: {idx}/{total} ({idx*100//total}%) | "
                          f"Found: {len(results)} with data")
                    sys.stdout.flush()
                
                # Save progress every 100 stocks
                if idx % 100 == 0 and results:
                    df = pd.DataFrame(results)
                    df.to_csv('shortable_data_temp.csv', index=False)
                    print(f"  💾 Saved progress")
            
        except Exception as e:
            if idx % 10 == 0:
                print(f"Progress: {idx}/{total} ({idx*100//total}%) | Errors encountered")
        
        # Small delay to avoid overwhelming IBKR
        await asyncio.sleep(0.1)
    
    try:
        ib.disconnect()
        print("\n✓ Disconnected from IBKR")
    except:
        pass
    
    return results

def main():
    # Load symbols
    print("Loading symbols from us_stock_symbols.csv...")
    try:
        symbols_df = pd.read_csv('us_stock_symbols.csv')
        symbols = symbols_df['symbol'].tolist()
        print(f"Found {len(symbols)} symbols")
    except Exception as e:
        print(f"Error loading symbols: {e}")
        return
    
    # Create new event loop
    try:
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
        results = loop.run_until_complete(scan_all_stocks(symbols))
        loop.close()
    except Exception as e:
        print(f"Error during scan: {e}")
        return
    
    if results:
        # Save final results
        df = pd.DataFrame(results)
        df = df.sort_values('shortable_shares', ascending=True)
        df.to_csv('shortable_data.csv', index=False)
        
        print(f"\n✓ Complete!")
        print(f"Total stocks scanned: {len(symbols)}")
        print(f"Stocks with data: {len(results)}")
        print(f"Saved to: shortable_data.csv")
        
        # Summary stats
        low_shortable = df[df['shortable_shares'] < 20000]
        print(f"\nStocks with < 20K shortable shares: {len(low_shortable)}")
        
        # Clean up temp file
        if os.path.exists('shortable_data_temp.csv'):
            os.remove('shortable_data_temp.csv')
    else:
        print("\n✗ No data collected")

if __name__ == '__main__':
    main()
